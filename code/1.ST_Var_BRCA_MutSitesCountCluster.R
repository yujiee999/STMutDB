...
data_dir<- paste(input,GSE_id,SAM_id,sep="/")
...
saveRDS(brain, paste(output_inter,GSE_id,SAM_id,"ST_seuratObject.rds",sep="/"))
###########################################################################
##########################先跑出p10以前的代码，然后##########################
###########################################################################
##########################基因突变位点数突变谱聚类###############################
...
#输出位点数突变谱的seurat对象
saveRDS(seurat_object,  paste(output_inter,GSE_id,SAM_id,"MutSitesCount_seuratObject.rds",sep="/"))

spot_gene_mutcount_matrix[, -1][spot_gene_mutcount_matrix[, -1] > 0] <- 1

write.table(spot_gene_mutcount_matrix, paste(output_inter,GSE_id,SAM_id,"spot_gene_01_matrix.txt",sep="/"), sep = "\t", quote = F)
##质量控制
###不过滤变异的细胞数目非常少的基因
#gene_count <- rowSums(spot_gene_mutcount_matrix[, -1] > 0)# 计算每个基因在多少个细胞中表达
#filtered_gene_data <- spot_gene_mutcount_matrix[gene_count >= 100, ]# 筛选出在100个或更多细胞中表达的基因
...
#输出01突变谱的seurat对象
saveRDS(seurat_object, paste(output_inter,GSE_id,SAM_id,"Mut01_seuratObject.rds",sep="/"))


##########################4-空转展示scsnv突变聚类类别###############################
#***********************************************************************************
#
seurat_object_1 <- readRDS(paste(output_inter,GSE_id,SAM_id,"Mut01_seuratObject.rds",sep="/"))
seurat_object_2 <- readRDS(paste(output_inter,GSE_id,SAM_id,"MutSitesCount_seuratObject.rds",sep="/"))
brain <- readRDS(paste(output_inter,GSE_id,SAM_id,"ST_seuratObject.rds",sep="/"))
...
saveRDS(brain, paste(output_inter,GSE_id,SAM_id,"ST_Mut_SeuratObject.rds",sep="/"))
...
#********************************************************
brain <- readRDS(paste(output_inter,GSE_id,SAM_id,"ST_Mut_SeuratObject.rds",sep="/"))
...
png(paste(output,GSE_id,SAM_id,"nCount_Spatial_viloin_blow.png",sep="/"), width = 674, height = 674)
...
png(paste(output,GSE_id,SAM_id,"nFeature_Spatial_viloin_blow.png",sep="/"), width = 674, height = 674)
...
# png(paste(output,GSE_id,SAM_id,"nCount_RNA_viloin.png",sep="/"), width = 337, height = 280)
# print(nCount_RNA)
# # 进行绘图操作
# dev.off()

png(paste(output,GSE_id,SAM_id,"nCount_SNV_viloin_blow.png",sep="/"), width = 674, height = 674)
print(nCount_RNA)
# 进行绘图操作
dev.off()
...
png(paste(output,GSE_id,SAM_id,"nFeature_SNV_viloin_blow.png",sep="/"), width = 674, height = 674)
print(nFeature_RNA)
# 进行绘图操作
dev.off()
...
png(paste(output,GSE_id,SAM_id,"nCount_SNV_slice_blow.png",sep="/"), width = 674, height = 674)
print(nCount_RNA)
# 进行绘图操作
dev.off()
...
png(paste(output,GSE_id,SAM_id,"nFeature_SNV_slice_blow.png",sep="/"), width = 674, height = 674)
print(nFeature_RNA)
# 进行绘图操作
dev.off()
#展示spot的mRNA分子数
...
png(paste(output,GSE_id,SAM_id,"nCount_Spatial_slice_blow.png",sep="/"), width = 674, height = 674)
print(nCount_Spatial)
# 进行绘图操作
dev.off()
#展示spot的基因数目
...
png(paste(output,GSE_id,SAM_id,"nFeature_Spatial_slice_blow.png",sep="/"),width = 674, height = 674)
print(nFeature_Spatial)
# 进行绘图操作
dev.off()
...
png(paste(output,GSE_id,SAM_id,"cluster_blow.png",sep="/"), width = 674, height = 674)
DimPlot(seurat_object, reduction = "umap", pt.size=0.5)
dev.off()

png(paste(output,GSE_id,SAM_id,"cluster_slice_blow.png",sep="/"), width = 674, height = 674)
...
saveRDS(seurat_object, paste(output_inter,GSE_id,SAM_id,"MutSites_01_SeuratObject.rds",sep="/"))
...
png(paste(output,GSE_id,SAM_id,"top5marker_heatmap.png",sep="/"), width = 337, height = 280)
DoHeatmap(seurat_object, features = top5$gene) + NoLegend()
dev.off()

png(paste(output,GSE_id,SAM_id,"top5marker_heatmap_blow.png",sep="/"), width = 674, height = 674)
DoHeatmap(seurat_object, features = top5$gene) + NoLegend()
dev.off()

png(paste(output,GSE_id,SAM_id,"top5marker_dotplot.png",sep="/"), width = 337, height = 280)
...
png(paste(output,GSE_id,SAM_id,"top5marker_dotplot_blow.png",sep="/"), width = 674, height = 674)
DotPlot(seurat_object, features = unique(top5$gene), scale = F) + RotatedAxis() + guides(shape=guide_legend(title = NULL),
                                                                                         color=guide_legend(title = 'Average Mutation'),
                                                                                         size=guide_legend(title = 'Percent Mutated'),
)
...
erich.go.BP = enrichGO(gene = x1,
                       ...
                       pvalueCutoff = 0.05,
                       qvalueCutoff = 0.05)
png(paste(output,GSE_id,SAM_id,"GO_enrichment.png",sep="/"), width = 1200, height = 674)
