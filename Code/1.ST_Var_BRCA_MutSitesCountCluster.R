...
hg <- bitr(x1,fromType="SYMBOL",toType=c("ENTREZID","ENSEMBL","SYMBOL"),OrgDb="org.Hs.eg.db")#ID转换

#hg2 = bitr(x2,fromType="SYMBOL",toType=c("ENTREZID","ENSEMBL","SYMBOL"),OrgDb="org.Hs.eg.db")

go_1 <- enrichGO(hg$ENTREZID,OrgDb = org.Hs.eg.db, 
                 ont='ALL',#("ALL",BP,CC,MF)
                 pAdjustMethod = 'BH',
                 pvalueCutoff = 1, 
                 qvalueCutoff = 1,
                 keyType = 'ENTREZID')#进行GO富集，确定P值与Q值得卡值并使用BH方法对值进行调整???


Enrichment <- seurat_object.markers %>%
  mutate(SYMBOL=rownames(.)) %>%#生成新的列
  inner_join(hg, by = "SYMBOL") %>%#拼接数据框
  dplyr::select(SYMBOL,ENTREZID,avg_log2FC)#选择想要的列

go <- data.frame(Category = go_1@ontology,
                 ID = go_1@result$ID,
                 Term = go_1@result$Description,
                 Genes = gsub("/", ", ", go_1@result$geneID),
                 adj_pval = go_1@result$p.adjust)

genelist <- data.frame(ID = Enrichment$SYMBOL,
                       logFC = Enrichment$avg_log2FC)

circ <- circle_dat(go, genelist)#整理数据格式
head(circ)
...
kegg <- enrichKEGG(hg2$ENTREZID, organism = 'hsa', 
                   keyType = 'kegg', 
                   pvalueCutoff = 0.5, 
                   pAdjustMethod = 'BH', 
                   minGSSize = 3, 
